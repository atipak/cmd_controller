...
#include <my_pkg/ControllerState.h>
...
int main(int argc, char** argv) {
  ...
  control_state_pub = nh.advertise<my_pkg::ControllerState>("controllerState", 10);

  // Subscribe to bebop velocity commands messages
  vel_sub = nh.subscribe("/bebop/cmd_vel", 10, &twist_callback);
  take_off_sub = nh.subscribe("/bebop/takeoff", 10, &take_off_callback);
  ...
  mapClient = nh.serviceClient<my_pkg::PointHeight>("point_height");
  ros::Rate rate(10.0);
...
void send_state_message() {
	my_pkg::ControllerState state_msg;
	...
	if (state == 1) {
		if (twist_msg.linear.x != 0 || twist_msg.linear.y != 0 || twist_msg.linear.z != 0 ||
			...
			state_msg.twist.linear.x = twist_msg.linear.x; 
			state_msg.twist.linear.y = twist_msg.linear.y;
			state_msg.twist.linear.z = twist_msg.linear.z;
			state_msg.twist.angular.x = twist_msg.angular.x;
			state_msg.twist.angular.y = twist_msg.angular.y;
			state_msg.twist.angular.z = twist_msg.angular.z;
		}
		else {
			// zero velocity
			state = 0;
...
float get_minimum_in_point(float x, float y, ros::ServiceClient mapClient) {
	  my_pkg::PointHeight::Request  req;
...
bool velocity_update() {
	...
	dt = (ros::Time::now() - prev_time).toSec();
  	...
  	tf::Vector3 temp_origin = tf::Vector3(twist_msg.linear.x, twist_msg.linear.y, twist_msg.linear.z);
  	double temp_yaw = twist_msg.angular.z * axis_direction_yaw;
